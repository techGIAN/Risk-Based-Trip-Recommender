...
class RiskMap():
    ...
    def compute_hex_risk(self):
        ...
        '''
        self.grid_gdf['hex_risk'] = [self.get_hex_dict() for cid in self.grid_gdf['cellID']]

    def get_hex_dict(self):
        '''
            Each hexagon will return a dict for each time unit as keys and risk as values
        '''
        hex_risk_dict = dict()

        # First attempt, each hexagon has a random risk
        for t in range(self.MAX_TIME):
            hex_risk_dict[t] = round(np.random.rand(),2)

        # Second attempt, each hexagon's risk depends on a) density of people in hex and b) duration
        # TO-DO later

        return hex_risk_dict
